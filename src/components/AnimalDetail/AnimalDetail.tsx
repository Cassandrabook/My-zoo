...
import { imageError } from "../Animal/Animal";
...
export const AnimalDetail = () => {
  ...
  useEffect(() => {
    const animal =
      animals.find((animal: IAnimal) => animal.id === Number(id)) || null;
    ...
    <>
      <div className="animalDetail">
        <div className="animalDetail__image">
          <img src={pet?.imageUrl} alt={pet?.name} onError={imageError} />
        </div>
        <div className="animalDetail__text-container">
          ...
          <p className="animalDetail__birthday">
            <span>Födelseår: </span>
            {pet?.yearOfBirth}
          </p>
          <p className="animalDetail__medicine">
            <span>Mediciner: </span>
            {pet?.medicine}
          </p>
          <p>
            <span>Matad senast: </span>
            {fedTime}
          </p>
          <button
            className="animalDetail__btn"
            disabled={buttondisabled}
            onClick={handleFeedClick}
          >
            {buttondisabled === true
              ? "Kom tillbaka om 3 timmar"
              : `Mata ${pet?.name}`}
          </button>
          <p></p>
        </div>
